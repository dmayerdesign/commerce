...
export class UserService extends CrudService<User> {
    ...
    public async register(user: User, res: Response): Promise<void> {
        ...
        try {
            ...
            const existingUser = await this.repository.findOne(User, { email: user.email.toLowerCase() }, [ this.userPopulateOptions ])
            ...
            if (!existingUser) {
                ...
                let savedUser = await this.repository.save(newUser)
    ...
    public async updateUser(id: string, update: any): Promise<ApiResponse<User>> {
        try {
            const { _doc } = await this.repository.updateById(User, id, update)
            const user = _doc
    ...
    public async verifyEmail(token: string): Promise<ApiResponse<User>> {
        try {
            const user = await this.repository.findOne(User, { emailVerificationToken: token }, [ this.userPopulateOptions ])
